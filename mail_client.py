...
def createConnection(ip, port):
    ...
    while True:
        ...
        if response == 0 and server_message.startswith("220") and server_message.endswith("Service Ready\r\n"):
            hostname = server_message.split(" ")[1]
            client.send(("HELO " + str(hostname)).encode('utf-8'))
            response += 1
        elif response == 1 and server_message.startswith("250 OK Hello " + str(hostname)):
            return True, client
        else:
            client.close() 
            awake = False
...
def sendMailToServer(client : socket, sender: str, receiver: str, subject: str, message: str) -> bool:
    ...
    while True:
        ...
        elif response == 2 and server_message.startswith('354 Enter mail, end with "." on a line by itself'):
            response += 1
            print('start message loading')
            print(message)
            for line in range(0, len(message)):
                client.send((message[line] + "\r\n").encode())
        elif response == 3 and server_message.startswith('250 OK Message accepted for delivery'):
...
def mailSending(ip) -> bool:
    ...
    formatted_correctly, error_message, sender, receiver, subject = messageFormatChecker(message)
...
def mailManagement(ip):
    ...
    for n in range(0, len(emails)):
        print(str(n) + "-" + emails[n]["From"] + "-" + emails[n]["Received"] + "-" + emails[n]["Subject"] + "\n")
            ...
            if len(command) > 1:
                if not command[1].isnumeric():
                    ...
                    if server_message.startswith("+OK"):
                        ...
                        print("Message: " + server_message[1] + " Size: " + server_message[2] + "\n")
                    else:
                ...
                if server_message == '.\r\n':
                    ...
                    server_message = server_message.split(" ")
                    print("Message count: " + server_message[1] + " Size: " + server_message[2] + " octets\n")
                    server_message = client.recv(1012).decode('utf-8')
                    while server_message != '.\r\n':
                        server_message = server_message.split(" ")
                        print("Message: " + server_message[1] + " Size: " + server_message[2] + " octets\n")
...
def messageFormatChecker(message):
    ...
    if len(format) < 4:
        ...
        return False, error_message, formatted_sender, formatted_receiver, formatted_subject
    ...
    if len(subject) != 2:
        error_message = "Expected 'Subject: <subject string>', but received '" + \
            str(format[2]) + "'."
        return False, error_message, formatted_sender, formatted_receiver, formatted_subject
...
if __name__ == "__main__":
    if len(sys.argv) - 1 != 1:
        raise Exception("SMTP mail server expects 1 argument, " +
                        str(len(sys.argv - 1)) + " were given.")
